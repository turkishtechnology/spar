...
import { ToastProvider, ToastViewport, toast, type SwipeDirection } from './index';
...
export const ToastSwipeDemo: React.FC = () => {
  ...
  const showMultipleSwipeableToasts = () => {
    toast.success('Swipe ← left to dismiss', {
      ...
      onSwipeEnd: (direction) => {
        // eslint-disable-next-line no-console
        console.log(`Success toast swiped ${direction}`);
      },
    });
    ...
    setTimeout(() => {
      toast.error('Swipe → right to dismiss', {
        ...
        onSwipeEnd: (direction) => {
          // eslint-disable-next-line no-console
          console.log(`Error toast swiped ${direction}`);
        },
      });
    }, 500);
    ...
    setTimeout(() => {
      toast.warning('Swipe ↑ up to dismiss', {
        ...
        onSwipeEnd: (direction) => {
          // eslint-disable-next-line no-console
          console.log(`Warning toast swiped ${direction}`);
        },
      });
    }, 1000);
  };
  ...
  const showCustomThresholdToast = () => {
    toast.info('Need to swipe 100px to dismiss (high threshold)', {
      ...
      onSwipeEnd: (direction) => {
        // eslint-disable-next-line no-console
        console.log(`Completed swipe ${direction} with high threshold`);
      },
    });
  };
  ...
  return (
    <div style={{ padding: '2rem', fontFamily: 'system-ui' }}>
      ...
      <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '2rem' }}>
        <button onClick={() => showSwipeableToast('success')}>Success Toast (Swipeable)</button>
        <button onClick={() => showSwipeableToast('error')}>Error Toast (Swipeable)</button>
        <button onClick={() => showSwipeableToast('warning')}>Warning Toast (Swipeable)</button>
        <button onClick={() => showSwipeableToast('info')}>Info Toast (Swipeable)</button>
      </div>

      <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '2rem' }}>
        <button onClick={showMultipleSwipeableToasts}>Show Multiple Toasts</button>
        <button onClick={showCustomThresholdToast}>Custom Threshold (100px)</button>
      </div>
      ...
      >
        ...
        <ul>
          <li>
            <strong>Mouse:</strong> Click and drag on a toast in any direction
          </li>
          <li>
            <strong>Touch:</strong> Swipe on a toast with your finger
          </li>
          <li>
            <strong>Threshold:</strong> Default requires 50px movement, custom can be set higher
          </li>
          <li>
            <strong>Directions:</strong> All 4 directions (up, down, left, right) are supported
          </li>
          <li>
            <strong>Feedback:</strong> Check console for swipe event logs
          </li>
        </ul>
      </div>
...
export const ToastSwipeApp: React.FC = () => {
  return (
    ...
    >
      <ToastSwipeDemo />
      <ToastViewport position='top-right' maxWidth={400} />
    </ToastProvider>
  );
};
