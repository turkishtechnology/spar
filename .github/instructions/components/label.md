...
| `optional` | `boolean` | No | `false` | Marks label for an optional field (exposed via data attribute) |
...
| `invalid` | `boolean` | No | `false` | Marks label for an invalid field (exposed via data attribute) |
...
**⚠️ Important Notes:**
- `required`, `optional`, `disabled`, `readOnly`, and `invalid` are for **styling purposes only**. Always set `required`, `aria-required`, `disabled`, `readOnly`, and `aria-invalid` attributes on the **form control itself** for proper functionality.
...
| Optional | Render with `optional={true}` | Marks as optional field label | `data-optional=""` |
...
| Invalid | Render with `invalid={true}` | Marks as invalid field label | `data-invalid=""` |
...
5. ✅ Add state props (`required`, `optional`, `disabled`, `readOnly`, `invalid`)
